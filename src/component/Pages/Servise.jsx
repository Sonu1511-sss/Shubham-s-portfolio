...
const services = [
  ...
  { id: 3, title: 'SOCAIL  MEDIA', description: 'Social media specialist driving online engagement...' },
...
const Servise = () => {
  return (
    <section className="bg-black py-12 mt-10">
      <h2 className="text-center text-4xl font-bold text-purple-500 mb-8">My <span className='text-white'>Services</span> </h2>
      <div className="container mx-auto grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
        {services.map((service) => (
          <div
            key={service.id}
            className="b p-6 rounded-lg border border-gray-200 cursor-pointer shadow-lg transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl"
          >
            <h3 className="text-2xl font-bold text-purple-500 mb-2">{String(service.id).padStart(2, '0')} {service.title}</h3>
